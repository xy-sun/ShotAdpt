...
class MimoModel(Module):
    def __init__(self, model: ProtoModule, fast_weight_lst) -> None:
        super().__init__()
        self.proto = model
        self.params_lsts = fast_weight_lst

    def forward(self, inputs_lst, eval_lst):
        output_lst = []
        for params, input, evaluator in zip(self.params_lsts, inputs_lst, eval_lst):
            evaluator = (lambda x, y: x(y)) if evaluator is None else evaluator
            out = evaluator(self.proto.functional(params), input)
            output_lst.append(out)
        return output_lst
